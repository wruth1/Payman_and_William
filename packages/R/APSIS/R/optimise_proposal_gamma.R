#' Optimised Adaptive Importance Sampler with a Family of Gamma Distribution for
#' Proposal Distribution
#'
#' @param target A target distribution. This must be a function of the sample
#'   (x) only. The `optimise_proposal_gamma` function will evaluate this target
...
#' @param lambda0 The starting value for the scale parameter in the proposal
#'   gamma distribution.
...
#'
optimise_proposal_gamma = function(target, lambda0, shape, n = 100, boundary = 2, control = NULL){
  ...
  while( (t < max.iter) | check ){
    ...
    param[t+1] <- update_proposal_gamma(n = n, lambda = param[t], f = target, a = shape, step.size = 1/t)
