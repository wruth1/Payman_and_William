...
In order to compute $\zeta$, we need to make a few intermediate steps. First, not that the derivative of the ratio in \eqref{eq:tail_defn} can be re-written as
%
\begin{align}
...
\end{lemma}
\begin{proof}
    The proof proceeds by routine, albeit messy, calculus and algebra. We start by applying the quotient and product rules to $\sW'$. For notational simplicity, we write $\sV(w) = 1 + \sW(w)$, and suppress the arguments from $\sW$ and $\sV$.
    %
    \begin{align}
        \sW'' &= \left( \frac{\sW}{w \sV} \right)'\\
        &= \frac{w \sV \sW' - \sW ( \sV + w \sV')}{w^2 \sV^2}\\
        &= \frac{\sW - \sW (\sV + w \sW')}{w^2 \sV^2}\\
        &= \frac{\sW}{w^2 \sV^2} ( 1 - \sV - \sW / \sV)\\
        &= \frac{\sW}{w^2 \sV^2} (- \sW - \sW / \sV)\\
        &= -\frac{\sW^2}{w^2 \sV^2} \left( \frac{2 + \sW}{1 + \sW} \right)\\
        &= - (\sW')^2 \left( \frac{2 + \sW}{1 + \sW} \right)
    \end{align}
\end{proof}

We are now equipped to start un-tangling the mess that is Equation \eqref{eq:wt_dens}. We start with $f'_u$. The structure of $f_l'$ will be analogous. 
...
\end{align}
%
This reduces $f'_u$ to an expression in terms of things we know how to compute, namely $X_u$ and $\sW_l''$. The latter of these has an expression in terms of $\sW_l$ and $\sW_l'$ (Equation \eqref{eq:W_prime_prime}). Finally, $\sW_l'$ can be written in terms of $\sW$ (Equation \eqref{eq:W_prime}). An analogous expression holds for $f_l'$.
